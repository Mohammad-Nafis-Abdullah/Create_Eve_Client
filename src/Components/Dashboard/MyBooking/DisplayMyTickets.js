...
const DisplayMyTickets = ({ ticket, refetch }) => {
    ...
    // const [event] = useRefetch(`/event-details/${ticket?.eventId}`, {});
    const { data: event } = useQueryFetch('event', `/event-details/${ticket?.eventId}`);
    // cancle order
    const handleBookingCancle = id => {
        ...
        if (proceed) {
            axios.delete(`/delete-booked-ticket/${id}`)
        ...
        >
            ...
            >
                <div className='space-y-2 basis-[15rem] shrink flex flex-col justify-center items-center'>
                    ...
                    <h3 className=" text-2xl openSans text-amber-500">{naming(event?.title)[0]}</h3>
                    <h3 className=" text-2xl openSans text-amber-500">{naming(event?.title)[1]}</h3>

                    <h3 className='font-medium text-white'>{addressing(event?.address)[0]}</h3>
                    <h3 className='font-medium text-white'>{addressing(event?.address)[1]}</h3>
                </div>

                <div className='space-y-2 basis-[15rem] shrink'>
                    <p className="text-xl openSans font-bold mb-2 text-amber-600">Ticket & Type</p>
                    ...
                    <div className="flex justify-end gap-5 px-5 pt-5">
                        <button onClick={() => handleBookingCancle(eventId)} className={`border-4 border-red-500 inline-block w-28 h-12 openSans uppercase tracking-wider transition-all text-red-500 font-extrabold hover:shadow-[0_0_35px_rgb(236,68,68)] hover:bg-red-500 hover:text-gray-900 text-lg duration-300`}>Cancel</button>
                        <button onClick={() => navigate(`/dashboard/payment/${bookingId}`)} className={`border-4 border-sky-500 inline-block w-28 h-12 openSans uppercase tracking-wider transition-all text-sky-500 font-extrabold hover:shadow-[0_0_35px_rgb(14,165,233)] hover:bg-sky-500 hover:text-gray-900 text-lg duration-300`}>Pay</button>
                    </div>
                </div>
            </section>
        </div>
